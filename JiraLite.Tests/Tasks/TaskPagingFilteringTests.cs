...
public class TaskPagingFilteringTests : TestBase
{
    ...
    public async Task Member_Can_Filter_And_Page_Tasks_Together()
    {
        ...
        payload.Items.Should().OnlyContain(t => t.Status == JiraTaskStatus.Todo);
    }
    ...
    {
        IEnumerable<TaskItem> Make(int n, JiraTaskStatus status) =>
        ...
        Db.Tasks.AddRange(Make(10, JiraTaskStatus.Todo));
        Db.Tasks.AddRange(Make(7, JiraTaskStatus.InProgress));
        Db.Tasks.AddRange(Make(3, JiraTaskStatus.Done));
